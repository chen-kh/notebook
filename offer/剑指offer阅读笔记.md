...
### 2.3. 字符串替换
将原本一个字符换成多个字符时，注意从右往左（减少移动次数），同样类型的还有归并排序，从后往前排
...
### 2.52. 链表中环的入口结点
分两步走，第一步确定环的数量，第二步两个指针，第一个指针先走n步(n就是环内结点数量)，然后两个指针齐头并进，相遇的时候就是入口结点。第一步确定环的数量，同样设定两个指针，第一个每次走一步，第二个每次走两步，相遇的时候就是在节点内（其实相遇的时候第一个节点走了几步就是有几个节点了）。然后可以循环一次算出节点数。
